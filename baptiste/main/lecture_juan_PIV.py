...
x = np.linspace(0, nx * mmparpixel_PIV / 1000, nx)  #Taille X en m
y = np.linspace(0, ny * mmparpixel_PIV / 1000, ny)  #Taille Y en m
...
plt.plot(x_pixel* mmparpixel_PIV / 10,y_pixel* mmparpixel_PIV / 10, 'mo')
figurejolie()
plt.plot(t, data[x_pixel,y_pixel,:])
plt.title('pos pixel en fct du tps')
fft_1 = fft.fft( data[x_pixel,y_pixel,:])
...
YY = fft.fft2(data - np.mean(data))
figurejolie()
plt.pcolormesh( np.abs(YY[:,1,:]))
cbar = plt.colorbar()
